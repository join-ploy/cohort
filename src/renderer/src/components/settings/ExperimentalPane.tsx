...
export function ExperimentalPane({
  ...
  const showGroupedWorkspaces = matchesSettingsSearch(searchQuery, [
    EXPERIMENTAL_SEARCH_ENTRY.groupedWorkspaces
  ])
    ...
    <div className="space-y-4">
      ...
      {showGroupedWorkspaces ? (
        <SearchableSetting
          ...
          keywords={EXPERIMENTAL_SEARCH_ENTRY.groupedWorkspaces.keywords}
          className="space-y-3 px-1 py-2"
        >
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0 shrink space-y-0.5">
              <Label>Grouped workspaces</Label>
              <p className="text-xs text-muted-foreground">
                Adds a Groups section above the repo list in the sidebar. The section hides itself
                when there are no groups to show. The grouped-workspaces UX is still under active
                development, so the data model and surface may change.
              </p>
            </div>
            <button
              type="button"
              role="switch"
              aria-checked={settings.experimentalGroupedWorkspaces}
              onClick={() =>
                updateSettings({
                  experimentalGroupedWorkspaces: !settings.experimentalGroupedWorkspaces
                })
              }
              className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer items-center rounded-full border border-transparent transition-colors ${
                settings.experimentalGroupedWorkspaces ? 'bg-foreground' : 'bg-muted-foreground/30'
              }`}
            >
              <span
                className={`inline-block h-3.5 w-3.5 transform rounded-full bg-background shadow-sm transition-transform ${
                  settings.experimentalGroupedWorkspaces ? 'translate-x-4' : 'translate-x-0.5'
                }`}
              />
            </button>
          </div>
